...
import { datasInternacionaisEm, mesDiaDeIso } from "@/lib/datas-internacionais";
...
export default function CalendarioAnual() {
  ...
  const datasInternacionaisDoDia = diaSelecionado
    ? datasInternacionaisEm(mesDiaDeIso(diaSelecionado))
    : [];
  ...
  function selecionarDia(key: string, mesIndex?: number) {
    if (typeof mesIndex === "number") setMesSelecionado(mesIndex);
  ...
  return (
    <div className="lg:grid lg:grid-cols-[7fr_3fr] lg:gap-5 lg:items-start">
                    ...
                    return (
                      <div key={key} className="flex flex-col items-start gap-0.5">
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            selecionarDia(key, mesIndex);
                          }}
                          aria-label={`${dia.getDate()} de ${nome}`}
                          className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] leading-none transition-colors ${
                            ehHoje
                              ? "bg-crimson text-white font-bold"
                              : ehSelecionado
                                ? "ring-1 ring-crimson font-bold text-navy-900"
                                : !noMes
                                  ? "text-navy-900/55 cursor-default"
                                  : fimDeSemana && !temEvento
                                    ? "text-navy-900/70 hover:bg-navy-100"
                                    : "text-navy-900 font-semibold hover:bg-navy-100"
                          }`}
                        >
                          {dia.getDate()}
                        </button>
      ...
      <aside className="mt-10 lg:mt-0 lg:sticky lg:top-24 bg-white border border-navy-100 overflow-hidden">
        <div className="relative">
          {(() => {
            const imgFundo =
              eventosDoDiaSelecionado.find((e) => e.imagemFundo)?.imagemFundo ||
              eventosDoDiaSelecionado[0]?.imagemFundo;
            ...
            return (
              <>
                ...
                />
                <div
                  className="pointer-events-none absolute inset-0 bg-gradient-to-b from-navy-900/45 via-navy-900/25 to-navy-900/50"
                  aria-hidden
                />
              </>
            );
          ...
          <div className="relative z-10 p-6 pb-5">
            ...
            <div className="rounded-xl border border-navy-100 bg-white/90 p-3">
              <div className="grid grid-cols-7 text-center">
                {DIAS_SEMANA.map((d, i) => (
                  <span key={i} className="text-[10px] font-bold text-navy-900/70 uppercase tracking-wide pb-1.5">
                    {d}
                  </span>
                ))}
        ...
        <div className="px-6 pb-6 pt-5 border-t border-navy-100 bg-white">
            {!diaSelecionado ? (
              ...
              <>
                        ...
                        <li key={ev.id}>
                          <div className="flex gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              {ev.descricao && (
                                <p className="text-xs text-navy-900/70 mt-0.5 leading-relaxed">{ev.descricao}</p>
                              )}
                            </div>
                          </div>
                          {ev.consideracaoEsj && (
                ...
                {(datasInternacionaisDoDia.length > 0 ||
                  eventosDoDiaSelecionado.some((ev) => ev.categoria === "feriado")) && (
                  <div className="mt-5 pt-4 border-t border-navy-100/80">
                    <p className="text-[10px] font-bold uppercase tracking-wide text-sky mb-2">
                      Outras Comemorações
                    </p>
                    {datasInternacionaisDoDia.length > 0 ? (
                      <ul className="space-y-1.5">
                        {datasInternacionaisDoDia.map((d) => (
                          <li key={d.titulo} className="flex items-start gap-2 text-xs text-navy-900/70">
                      ...
                      <p className="text-xs text-navy-900/50">
                        Sem outra comemoração internacional registada neste dia.
                      </p>
                    )}
                  </div>
                )}
