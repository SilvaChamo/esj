"use client";

import Image from "next/image";
import { FormEvent, useState } from "react";
import { MapPin, Phone } from "lucide-react";
import EntradaHome from "@/components/EntradaHome";
import NewsletterSeccao from "@/components/NewsletterSeccao";

const fieldClass =
  "w-full bg-transparent border border-white/80 px-3 text-sm text-white placeholder:text-white/80 outline-none focus:border-white";

function growMessageField(el: HTMLTextAreaElement) {
  el.style.height = "auto";
  el.style.height = `${Math.max(el.scrollHeight, 144)}px`;
}

export default function ContactBand() {
  const [msgState, setMsgState] = useState<"idle" | "busy" | "ok" | "err">("idle");

  const sendMessage = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const form = e.currentTarget;
    const data = new FormData(form);
    setMsgState("busy");
    try {
      const res = await fetch("/api/contacto", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tipo: "mensagem",
          nome: String(data.get("nome") || "").trim(),
          email: String(data.get("email") || "").trim(),
          mensagem: String(data.get("mensagem") || "").trim(),
        }),
      });
      if (!res.ok) throw new Error("fail");
      setMsgState("ok");
      form.reset();
      const ta = form.querySelector("textarea");
      if (ta) {
        ta.style.height = "";
      }
    } catch {
      setMsgState("err");
    }
  };

  return (
    <section id="contacto" className="scroll-mt-[124px] bg-cream">
      <div className="mx-auto max-w-7xl px-4 pt-16 pb-6 md:pt-24 md:pb-8">
        <div className="mb-[72px]">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_360px] md:items-stretch">
            <EntradaHome className="w-full min-h-0">
              <form
                onSubmit={sendMessage}
                className="relative z-0 overflow-hidden flex flex-col w-full h-full"
              >
                <Image
                  src="/JornalistaII.jpg"
                  alt=""
                  fill
                  sizes="(max-width: 1280px) 100vw, 1280px"
                  aria-hidden
                  className="-z-10 object-cover object-[50%_30%]"
                />
                <div className="absolute inset-0 -z-10 bg-[#1a5a78]/90" aria-hidden />
                <div className="absolute inset-0 -z-10 bg-[#1a5a78]/35" aria-hidden />

                <div className="relative z-0 bg-navy-900/45 px-8 sm:px-10 pt-8 sm:pt-10 pb-6 sm:pb-7">
                  <p className="flex items-center gap-3 text-white font-bold tracking-widest text-sm mb-3">
                    <span className="h-px w-[40px] shrink-0 bg-white" aria-hidden />
                    CONTACTO
                  </p>
                  <h2 className="font-serif text-3xl md:text-[2.05rem] font-bold text-navy-900">
                    <span className="text-white">Fale</span> <span className="text-sky">connosco</span>
                  </h2>
                  <p className="mt-4 text-sm text-white/85 leading-relaxed">
                    A Escola Superior de Jornalismo está disponível para esclarecer dúvidas sobre
                    admissões, cursos e parcerias. Contacte a Secretaria Académica pelos meios ao
                    lado ou preencha o formulário abaixo e responderemos o mais breve possível.
                  </p>
                </div>

                <div className="relative z-0 h-px w-full bg-sky-300" aria-hidden />

                <div className="relative z-0 flex flex-col flex-1 justify-center px-8 sm:px-10 pt-6 sm:pt-7 pb-8 sm:pb-10">
                  <div className="grid sm:grid-cols-2 gap-3 flex-1 content-start">
                    <input
                      name="nome"
                      required
                      autoComplete="name"
                      placeholder="Nome"
                      className={`${fieldClass} h-11`}
                    />
                    <input
                      name="email"
                      type="email"
                      required
                      autoComplete="email"
                      placeholder="Correio"
                      className={`${fieldClass} h-11`}
                    />
                    <textarea
                      name="mensagem"
                      required
                      rows={6}
                      placeholder="Mensagem"
                      onInput={(e) => growMessageField(e.currentTarget)}
                      className={`${fieldClass} sm:col-span-2 min-h-[9rem] py-2.5 leading-relaxed resize-y overflow-hidden`}
                    />
                  </div>
                  <div className="mt-6 flex justify-end">
                    <button
                      type="submit"
                      disabled={msgState === "busy"}
                      className="esj-btn-move h-11 px-6 bg-white text-navy-900 text-[11px] font-bold tracking-[0.14em] hover:text-white disabled:opacity-60"
                    >
                      {msgState === "busy" ? "A ENVIAR…" : "ENVIAR MENSAGEM"}
                    </button>
                  </div>
                  {msgState === "ok" && (
                    <p className="mt-4 text-sm text-white text-right">Mensagem recebida. Obrigado.</p>
                  )}
                  {msgState === "err" && (
                    <p className="mt-4 text-sm text-white text-right">Não foi possível enviar. Tente de novo.</p>
                  )}
                </div>
              </form>
            </EntradaHome>

            <EntradaHome atraso={0.1} className="w-full min-h-0">
              <aside className="relative z-0 flex flex-col justify-center bg-navy-800 text-white w-full h-full">
                <dl className="p-8 sm:p-10 divide-y divide-white/20">
                  <div className="pb-4 flex gap-3">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center border border-sky-300">
                      <MapPin size={16} className="text-sky-300" />
                    </span>
                    <div className="min-w-0">
                      <dt className="font-semibold text-sm mb-1">Direcção Geral</dt>
                      <dd className="text-[11px] text-white/90 leading-snug">
                        Av. de Moçambique, Km 14, Recinto do Estádio Nacional do Zimpeto, Bloco C.
                        Maputo, Moçambique
                      </dd>
                    </div>
                  </div>
                  <div className="py-4 flex gap-3">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center border border-sky-300">
                      <MapPin size={16} className="text-sky-300" />
                    </span>
                    <div className="min-w-0">
                      <dt className="font-semibold text-sm mb-1">Registo Académico</dt>
                      <dd className="text-[11px] text-white/90 leading-snug">
                        Av. Ho Chi Min 103, Edifício da Escola de Jornalismo. Maputo, Moçambique
                      </dd>
                    </div>
                  </div>
                  <div className="py-4 flex gap-3">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center border border-sky-300">
                      <Phone size={16} className="text-sky-300" />
                    </span>
                    <div className="min-w-0">
                      <dt className="font-semibold text-sm mb-1">Contacto</dt>
                      <dd className="text-[11px] text-white/90 leading-snug">
                        <a href="tel:+25821302721" className="hover:underline">
                          00 258 21302721
                        </a>
                      </dd>
                    </div>
                  </div>
                  <div className="py-4 flex gap-3">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center border border-sky-300">
                      <MapPin size={16} className="text-sky-300" />
                    </span>
                    <div className="min-w-0">
                      <dt className="font-semibold text-sm mb-1">Salas de aulas</dt>
                      <dd className="text-[11px] text-white/90 leading-snug">
                        Av. Ahmed Sekou Touré, Edifício do Instituto Industrial 1º de Maio
                      </dd>
                    </div>
                  </div>
                  <div className="pt-4 flex gap-3">
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center border border-sky-300">
                      <MapPin size={16} className="text-sky-300" />
                    </span>
                    <div className="min-w-0">
                      <dt className="font-semibold text-sm mb-1">Delegação Académica – Manica</dt>
                      <dd className="text-[11px] text-white/90 leading-snug">
                        Bairro Tembwe – Chimoio, Recinto do Centro Provincial de Educação à Distância
                        de Manica
                      </dd>
                    </div>
                  </div>
                </dl>
              </aside>
            </EntradaHome>
          </div>
        </div>
      </div>

      <NewsletterSeccao />
    </section>
  );
}
